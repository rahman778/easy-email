...
export const MergeTags: React.FC<{ onChange: (v: string) => void; value: string; isSelect?: boolean; type?: string; table?: boolean }> = React.memo(
   (props) => {
      ...
      const treeOptions = useMemo(() => {
         ...
         const deep = (key: string, title: string, parent: { [key: string]: any; children?: any[] }, mapData: Array<any> = []) => {
            const currentMapData = {
               key: props.table ? `<%${key}%>` : `{{${key}}}`,
               value: props.table ? `<%${key}%>` : `{{${key}}}`,
               title: title,
               children: [],
            };
