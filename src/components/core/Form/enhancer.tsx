...
export default function enhancer<P>(Component: any, changeAdapter: (e: any) => any) {
   return (props: EnhancerProps<P> & Omit<P, "value" | "onChange" | "mutators">) => {
      const {
         name,
         onChangeAdapter,
         valueAdapter,
      ...
      return (
         <Form.Item style={{ margin: 0 }} validateStatus={touched && error ? "error" : undefined} help={touched && error}>
                  ...
                  <Stack.Item>
                     <label className={labelHidden ? styles["label-hidden"] : undefined} htmlFor={id}>
                        <span style={{ whiteSpace: "pre" }}>
                           ...
                           <TextStyle size={size === "small" ? "smallest" : "small"}>{label}</TextStyle>
                        </span>
                     </label>
                  </Stack.Item>
