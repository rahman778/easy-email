   ...
   const execCommand = (cmd: string, val?: any) => {
      ...
      if (cmd === "createLink") {
         ...
         if (linkData.linkNode) {
            ...
            link = getShadowRoot().querySelector(`a[href="${uuid}"`)! as HTMLAnchorElement;
         }
      ...
      <div id="TextToolbar" style={{ display: "flex", flexWrap: "nowrap" }}>
         <Stack vertical spacing="extraTight">
            <Stack spacing="extraTight">
               <Tooltip color="#fff" title={<FontFamily onChange={(val) => execCommand("fontName", val)} />} getPopupContainer={getPopoverMountNode}>
                  <Button size="small" icon={<IconFont iconName="icon-font" />} />
               </Tooltip>
               <Tooltip
                  color="#fff"
                  title={<FontSizeList onChange={(val) => execCommand("fontSize", val)} />}
               ...
               <Link currentRange={currentRange} onChange={(values) => execCommand("createLink", values)} getPopupContainer={getPopoverMountNode} />
               {/* <ToolItem
          ...
          /> */}
               {mergeTags && (
                  <Tooltip
                     color="#fff"
                     placement="bottom"
                     title={<MergeTags value="" onChange={(val) => execCommand("insertHTML", val)} />}
                     getPopupContainer={getPopoverMountNode}
                  >
