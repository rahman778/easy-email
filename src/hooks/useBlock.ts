...
import { findBlockByType, getIndexByIdx, getPageIdx, getParentByIdx, getParentIdx, getValueByIdx } from "@/utils/block";
   ...
   const addBlock = useCallback(
      (params: { type: BlockType; parentIdx: string; positionIndex?: number; payload?: any; canReplace?: boolean }) => {
         ...
         if (!fixedBlock.validParentType.includes(parent.type)) {
            message.warning(`${block.type} cannot be used inside ${parentBlock.type}, only inside: ${block.validParentType.join(", ")}`);
            return;
         }
