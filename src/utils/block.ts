...
export function getNodeTypeFromClassName(classList: DOMTokenList | string): BlockType | null {
...
export const getValueByIdx = <T extends IBlockData>(values: { content: IPage }, idx: string): T | null => {
   return get(values, idx);
};

export const getParentByIdx = <T extends IBlockData>(values: { content: IPage }, idx: string): T | null => {
...
export const getParentByType = <T extends IBlockData>(context: { content: IPage }, idx: string, type: BlockType): T | null => {
   if (!idx) return null;
...
export const getValidChildBlocks = (type: BlockType): IBlock[] => {
   return BlocksMap.getBlocks().filter((item) => item.validParentType.includes(type));
};
